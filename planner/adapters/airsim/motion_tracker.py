...
class AirSimMotionTracker:
    ...
    def execute(self, segment: MotionSegment) -> None:
        ...
        limits = self.limits
        generation = self.observation_gate.generation if self.observation_gate else 0
        ...
        settled_since = None
        while True:
            guard = self.observation_gate.step() if self.observation_gate else nullcontext(generation)
            with guard as current_generation:
                resumed = generation != current_generation
                generation = current_generation
                if resumed:
                    start = self.current_pose()
                    duration = duration_for(start, segment.end, speed, limits)
                    if duration + limits.settle_time_s >= self.timeout_s:
                        raise ExecutionError("resumed motion exceeds execution timeout")
                    segment = replace(segment, start=start, duration_s=duration)
                    began = previous_time = self.clock()
                    previous_pose = start
                    last_velocity, last_yaw_rate = (0.0, 0.0, 0.0), 0.0
                    settled_since = None
                    logger.info("[Motion] resume from measured pose toward original target: %s", segment.end)
                tick = self.clock()
                elapsed = tick - began
