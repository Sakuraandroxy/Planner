...
每条轨迹最多动作数：{max_trajectory_length}

任务目标：{task_description}
{depth_info}
输出{k}条候选轨迹，每条轨迹最多 {max_trajectory_length} 个动作，JSON格式：
{{
  "selected_index": 0,
  "done": false,
...
3. 如果没有目标bbox深度，则参考画面中心/全局深度统计，避免盲目前进
4. 如果目标深度大于单次最大前进距离，可以在同一条候选轨迹中连续输出多个 forward；每个 forward 不得超过 {max_forward_step} 米，每条轨迹最多 {max_trajectory_length} 个动作，总前进距离不得超过目标最近深度，并建议保留1~2米安全距离
...
TASK_INSTRUCTION_PROMPT = "任务：{task_description}。{depth_info}。生成 {k} 条候选轨迹；每条轨迹最多 {max_trajectory_length} 个动作；远目标可用多个连续 forward 分段接近，但每段不得超过单次最大前进距离，用中文输出。"
...
3. 如果没有目标bbox深度，则参考画面中心/全局深度统计，避免盲目前进
4. 任何单个 forward 数值不得超过 {max_forward_step} 米；目标很远时可以连续输出多个 forward 分段接近，例如 forward {max_forward_step}、forward {max_forward_step}、forward 4；每条轨迹最多 {max_trajectory_length} 个动作，总前进距离不得超过目标最近深度，并建议保留1~2米安全距离
...
VALUE_TASK_INSTRUCTION_PROMPT = "任务：{task_description}。{depth_info}。生成 {k} 条候选轨迹，每条轨迹最多 {max_trajectory_length} 个动作，每个动作带精确值（如 forward 4.8）；远目标可用多个连续 forward 分段接近，但每段不得超过单次最大前进距离，用中文输出。"
