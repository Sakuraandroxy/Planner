"""Candidate trajectory generation and perturbation."""
...
from agent.functions.candidate.base import CandidateTrajectory, actions_to_cumulative_body_waypoints


def build_seed_candidates(result) -> List[CandidateTrajectory]:
    """Build seed candidates from planner output."""
...
def generate_perturbed_candidates(
    ...
    """Generate rigid perturbations around one seed trajectory."""
...
def generate_smooth_bridge_candidates(
    ...
    """Generate smooth bridge candidates with fixed start/end points."""
    if not seed.waypoints or max_candidates <= 0:
        return []
    ...
    for _ in range(attempts):
        ...
        for noise_idx, path_idx in enumerate(free_indices):
            ...
            if not _is_xyz_point(base_wp) or not _is_xyz_point(normal):
                valid_attempt = False
                break
            new_path[path_idx] = [
...
def _sample_bridge_noise(
    ...
    # Gaussian bridge: zero noise at start/end and larger noise near the middle.
