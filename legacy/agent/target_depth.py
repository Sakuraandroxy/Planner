...
# ═══════════════════════════════════════════════════════════════
# GroundingDINO 集成（替换 VLM 检测）
# ═══════════════════════════════════════════════════════════════

def detect_with_groundingdino(image, caption, depth_meters=None,
                               bbox_image_size=None) -> TargetDepthEstimate:
    """使用 GroundingDINO 替代 VLM 做目标检测。

    调用服务器上的 GroundingDINO 服务，返回与现有 TargetDepthEstimate
    兼容的结果，可直接喂给 estimate_depth_in_bbox() 和 choose_target_estimate()。

    Args:
        image: PIL RGB Image
        caption: 检测目标描述（如 "red car", "person"）
        depth_meters: 深度图 (H, W)，可选
        bbox_image_size: RGB 图尺寸 (w, h)，可选

    Returns:
        TargetDepthEstimate 对象
    """
    from agent.detector.groundingdino_detector import GroundingDINODetector

    # 1. 提取英文物体名，调用 GroundingDINO 服务
    effective_caption = caption  # VLM 任务解析已输出英文
    det = GroundingDINODetector()
    detection = det.detect(image, effective_caption)

    # 2. 构建 TargetDepthEstimate
    if not detection.visible or detection.bbox is None:
        return TargetDepthEstimate(
            visible=False,
            target=_normalize_gdino_phrase(caption, detection.phrase),
        )

    x1, y1, x2, y2 = detection.bbox
    w, h = image.size

    # GroundingDINO 返回像素坐标，转为归一化 [0,1] raw_bbox
    raw_norm = [x1/w, y1/h, x2/w, y2/h]

    estimate = TargetDepthEstimate(
        visible=True,
        bbox=detection.bbox,                      # 像素坐标 [x1,y1,x2,y2]
    ...
    # 深度在 pipeline 中统一由 estimate_depth_in_bbox 计算
    return estimate


def detect_scene_with_groundingdino(image, caption, depth_meters=None,
                                     bbox_image_size=None) -> list[TargetDepthEstimate]:
    """使用 GroundingDINO 做场景级多物体检测。

    返回所有检测到的物体（含目标 + 障碍物），兼容现有 scene 分析流程。

    Args:
        image: PIL RGB Image
        caption: 场景描述（如 "car, tree, house, road"）
        depth_meters: 深度图，可选
        bbox_image_size: RGB 图尺寸，可选

    Returns:
        TargetDepthEstimate 列表
    """
    from agent.detector.groundingdino_detector import GroundingDINODetector

    effective_caption = caption  # VLM 任务解析已输出英文
    detections = _detect_all(image, effective_caption)
    ...
    return estimates if estimates else [TargetDepthEstimate(visible=False)]


def _normalize_gdino_phrase(task_caption: str, detected_phrase: str) -> str:
    """把 GroundingDINO 检测到的短语转为与任务一致的类名。"""
    phrase = (detected_phrase or "").strip()
    if not phrase:
        return task_caption
    return phrase
